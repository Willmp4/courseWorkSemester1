...
public class onlineShopSimple {
	...
	static void menu() {
		...
		for (int i = 1; i < prices.size() + 1; i++) {
			System.out.println("+  " + i + "." + "\t" + items.get(i) + "\t" + "£" + prices.get(items.get(i)));
	...
	public static void run() {
		...
		if (!items_map.containsKey(items.get(choice))) {
			...
			// Update the HashMap
			System.out.println("How much more " + items.get(choice) + " do you want?");
	...
	public static void receipt(String title) {
		...
		for (int i = 0; i < items_map.size(); i++) {
			System.out.println("*" + items_list.get(i) + "  " + items_map.get(items_list.get(i)) + "\t" + " £" +
			...
			// which is by also using the .get methode which crossfrences using the iteams
